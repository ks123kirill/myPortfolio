...
modalsCollection.forEach((item) => {
  item.addEventListener('click', function(e) {
    ...
    modal.setContent(
      ...
      <a class="last-works__link last-works__link--demo" href="${lastProjectsList[myProject].demo}" target="_blank" rel="noopener noreferrer" tabindex=1;>Посмотреть демо</a>
      <a class="last-works__link last-works__link--repo" href="${lastProjectsList[myProject].repo}" target="_blank" rel="noopener noreferrer" tabindex=1;>Посмотреть репозиторий</a>`
    ...
    window.addEventListener('keydown', function(evt) {
      if(evt.keyCode === 27 && pageBody.classList.contains('page-body--no-scroll')) {
      evt.preventDefault();
      modal.hide();
      }
    });
